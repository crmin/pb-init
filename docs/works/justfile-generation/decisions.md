...
- `--recommend`은 `--just`를 포함 - 2026-07-02 추가 요구에 따라 `--docker --auto-migration --just`와 동일하게 동작해야 함.
...
- `snapshot` 정리 대상은 configured migration dir - 2026-07-02 추가 요구에 따라 `--migration-dir` 경로와 cleanup 경로를 일치시켜야 함.
...
- Changed: `--recommend`은 `--just`를 포함하고, `snapshot` 정리 대상은 `--migration-dir` 경로를 사용하도록 결정 변경.
- Reason: 사용자가 후속 요구사항으로 recommended flag의 `--just` 포함과 custom migration directory cleanup을 명시했기 때문.
